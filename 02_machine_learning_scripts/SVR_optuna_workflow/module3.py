...
from sklearn.model_selection import LeaveOneOut  # Import LeaveOneOut for cross-validation
...
def main():
    parser = argparse.ArgumentParser(description='SVR Model Training with MLflow and Leave-One-Out Cross-Validation')
...
def evaluate_model(model, X, y):
    ...
    pearson_corr = pearsonr(y, y_pred)[0] if y.size > 1 else float('nan')
...
def compute_global_metrics(y_true, y_pred):
    ...
    pearson_corr = pearsonr(y_true, y_pred)[0] if y_true.size > 1 else float('nan')
    metrics = {
        'RMSE': rmse,
        'MAE': mae,
        'MAE StDev': mae_std,
...
def save_metrics_and_params(global_metrics, params, file_name, logger, final_metrics=None):
    with open(file_name, 'w', encoding='utf-8') as f:
        f.write(f"Model Status: Trained new model with Leave-One-Out Cross-Validation\n\n")
...
def process_file(csv_file, input_directory):
    try:
        ...
        with mlflow.start_run(run_name=f"Training_{csv_file}_{int(time.time())}"):
            for tag_name, tag_value in tags_config.mlflow_tags3.items():
            ...
            # Initialize predictions array for LOO CV
            ...
            # Compute global metrics over all predictions
            global_metrics = compute_global_metrics(y, y_pred_all)
            ...
            final_metrics_prefixed = {f'Final_{k}': v for k, v in final_metrics.items()}
            mlflow.log_metrics(final_metrics_prefixed)
            ...
            save_metrics_and_params(global_metrics, best_params, metrics_file_name, logger, final_metrics=final_metrics)
            mlflow.log_artifact(metrics_file_name)

            logger.info(
                f"Global Metrics for {csv_file}: RMSE: {global_metrics['RMSE']:.4f}, "
                f"R2: {global_metrics['R2']:.4f}, Pearson: {global_metrics['Pearson']:.4f}, "
                f"MAE: {global_metrics['MAE']:.4f}, MAE StDev: {global_metrics['MAE StDev']:.4f}"
            )
