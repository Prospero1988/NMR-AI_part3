# -*- coding: utf-8 -*-
"""
Created on Fri Sep 20 08:05:53 2024
...
def log_system_metrics():
    ...
    mlflow.log_metric('Memory_Usage', memory)
    mlflow.log_metric('GPU_Load', gpu_load)
    mlflow.log_metric('GPU_Memory_Usage', gpu_memory)
...
def optimize_hyperparameters(X, y, logger, csv_file, initial_model=None):
    ...
    def objective(trial):
        ...
        # Start with params from initial model if provided, otherwise use defaults
        if initial_model:
            logger.info("Using parameters from initial model as starting point for optimization.")
            params = initial_model.get_params()
        else:
            logger.info("No initial model provided, using default parameters.")
            params = {
                'verbosity': 0,
        ...
        # Now, allow Optuna to suggest new values for these parameters, even if initial_model is provided
        params['max_depth'] = trial.suggest_int('max_depth', 1, 20, default=params.get('max_depth', 6))
        params['learning_rate'] = trial.suggest_float('learning_rate', 1e-5, 0.5, log=True, default=params.get('learning_rate', 0.1))
        params['subsample'] = trial.suggest_float('subsample', 0.1, 1.0, default=params.get('subsample', 0.8))
        params['colsample_bytree'] = trial.suggest_float('colsample_bytree', 0.1, 1.0, default=params.get('colsample_bytree', 1.0))
        params['colsample_bylevel'] = trial.suggest_float('colsample_bylevel', 0.1, 1.0, default=params.get('colsample_bylevel', 1.0))
        params['colsample_bynode'] = trial.suggest_float('colsample_bynode', 0.1, 1.0, default=params.get('colsample_bynode', 1.0))
        params['min_child_weight'] = trial.suggest_float('min_child_weight', 0.1, 10.0, default=params.get('min_child_weight', 1.0))
        params['gamma'] = trial.suggest_float('gamma', 0.0, 5.0, default=params.get('gamma', 0.0))
        params['reg_lambda'] = trial.suggest_float('reg_lambda', 1e-8, 100.0, log=True, default=params.get('reg_lambda', 1.0))
        params['reg_alpha'] = trial.suggest_float('reg_alpha', 1e-8, 100.0, log=True, default=params.get('reg_alpha', 0.0))
        params['max_delta_step'] = trial.suggest_float('max_delta_step', 0.0, 10.0, default=params.get('max_delta_step', 0.0))
        params['grow_policy'] = trial.suggest_categorical('grow_policy', ['depthwise', 'lossguide'], default=params.get('grow_policy', 'depthwise'))
...
def process_file(csv_file, input_directory, model_uri=None):
    try:
        ...
        with mlflow.start_run(run_name=f"Run_{csv_file}"):
            ...
            # Load model from MLflow if URI is provided
            initial_model = None
            if model_uri:
                logger.info(f"Loading model from URI: {model_uri}")
                initial_model = mlflow.xgboost.load_model(model_uri)
...
def main():
    ...
    parser.add_argument('--model_uri', type=str, default=None, help='MLflow model URI for initial model')
