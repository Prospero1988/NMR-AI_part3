...
# -*- coding: utf-8 -*-
"""
CNN-Dual-Stream (¹H + ¹³C) with Optuna, full MLflow integration, 3CV → 10CV,
final training and artifact logging. Architecture:

  • ¹H stream : Conv1D-blockᵢ → … → embedding_h
  • ¹³C stream: Conv1D-blockᵢ → … → embedding_c
             └────── concat([embedding_h, embedding_c]) → (B, C*, L*)
                         └─ optional shared Conv1D block
                               └─ Flatten → FC → ŷ

Author: aleniak  | 07-2025
"""
...
def load_nmr_data(path_1h: str, path_13c: str) -> Tuple[pd.Series, np.ndarray, np.ndarray]:
    """
    ...
    # Pairing 1-to-1 with duplicates preserved (cumcount)
...
# ---------------------------------------------------------------------------
# CNN Dual-Stream Model
# ---------------------------------------------------------------------------
def conv_block_1d(in_ch: int, out_ch: int, k: int, s: int, p: int, dropout: float, use_bn: bool = True) -> List[nn.Module]:
...
def cross_validate(model_func, dataset, device, batch_size=64, n_folds=10, epochs=50):
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(indices)):
        ...
        for ep in range(epochs):
            train_one_epoch(model, tr_loader, optimizer, loss_fn, device)
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(res_dir, "script_cnndual.log")),
            logging.StreamHandler(sys.stdout)
        ]
    )
    logger = logging.getLogger(__name__)
    logger.info("Start CNNDual | results folder: %s", res_dir)
    ...
    try:
        ...
        logger.info("Best parameters: %s | RMSE=%.4f", best_params, best_value)
        ...
        with mlflow.start_run(run_name="CNNDual_10CV") as run:
            ...
            # RMSE plot
            plt.figure()
            plt.plot(df_trials["number"], df_trials["value"], "o-")
            plt.xlabel("Trial ID")
            plt.ylabel("RMSE (3CV)")
            plt.title("Optuna RMSE vs Trial ID (CNNDual)")
            plt.grid(True)
            ...
            for ep in range(args.epochs_10cv):
                train_one_epoch(final_model, loader, final_optimizer, loss_fn, device)
            ...
            # Feature matrix for leverage – concatenate 1H and 13C spectra
        ...
        logging.exception("Error in CNNDual script")
        sys.exit(1)
