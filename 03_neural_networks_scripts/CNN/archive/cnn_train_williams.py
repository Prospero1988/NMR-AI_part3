...
and to create an additional artefact: <input>_williams.csv with columns
...
def load_data(csv_path, target_column_name="LABEL"):
    """
    Wczytuje dane, zrzuca pierwszą kolumnę (nazwy cząsteczek) – przy uczeniu
    niepotrzebna, ale zostanie użyta później do pliku Williams.
    """
    data = pd.read_csv(csv_path)
    data_no_name = data.drop(data.columns[0], axis=1)  # wszystko oprócz MOLECULE_NAME
    ...
    with mlflow.start_run(run_name=csv_name):
        ...
        for epoch in range(epochs):
            ...
            for xb, yb in loader:
                ...
                optimizer.zero_grad()
                if xb.size(0) == 1:  # unikamy BN na batch=1
                    continue
        ...
        try:
            ...
            mol_names = original_df.iloc[:, 0]  # pierwotne MOLECULE_NAME
...
def main():
    parser = argparse.ArgumentParser(
        description="Train CNN models (no Optuna) and create _williams CSV artefact."
    )
