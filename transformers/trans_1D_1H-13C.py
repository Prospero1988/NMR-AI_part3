...
# -*- coding: utf-8 -*-
"""
Transformer 1D (hybryda ¹H + ¹³C) w formacie (N,1,2,200).
Positional Encoding + Optuna + 3CV->10CV, analogia do CNN2D alt.

Zmiany:
- Wymuszamy, żeby d_model było parzyste i wielokrotnością nhead.
- Usunięto duplikującą się definicję NMRDataset.
"""
...
# ---------------------------------------------------------------------------------
# Tagi MLflow (opcjonalne)
# ---------------------------------------------------------------------------------
MLFLOW_TAGS = {
    "property": "CHI logD",
    "model": "Transformer 1D (Hybrid 1H+13C, alt)",
    "predictor": "1Hx13C"
}
...
class NMRDataset(Dataset):
    ...
    Gdzie 2 = wiersze (1H, 13C), 200 = szerokość widma.
    """
...
def load_nmr_data(path_1h, path_13c):
    """
    Ładuje i łączy dane ¹H oraz ¹³C.
    Wynik: x_nmr o kształcie (N,1,2,200)
    """
    df_1h = pd.read_csv(path_1h)
    df_13c = pd.read_csv(path_13c)

    # Załóżmy, że kolumny to: MOLECULE_NAME, LABEL, a dalej 200 cech
...
class PositionalEncoding(nn.Module):
    """
    Klasyczna implementacja sinusoidalnego PE.
    Zakłada parzyste d_model (co i tak zapewnimy z zewnątrz).
    """
...
# =================================================================================
# Transformer 1D (hybryda 1H + 13C)
# =================================================================================
class TransformerRegressorHyb(nn.Module):
    """
    - Wejście: (B,1,2,200)
      => (B,2,200)
      => transponujemy na (B,200,2).
    - Embedding: Linear(2 -> d_model) => (B,200,d_model)
    - PositionalEncoding
    - TransformerEncoder
    - Global avg pooling => warstwa końcowa (regresja).
    """
    ...
    def forward(self, x):
        ...
        # 3) Embedding: (B,200,2) => (B,200,d_model)
        x = self.embedding(x)
        ...
        # 6) Global avg pooling => (B,d_model)
        x = x.mean(dim=1)
...
# =================================================================================
# Funkcje trenowania i walidacji
# =================================================================================
...
def create_model_and_optimizer(trial):
    model = TransformerRegressorHyb(trial)
...
def cross_validate(model_func, dataset, device, batch_size=64, n_folds=10, epochs=50):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(indices)):
        ...
        # Ostateczna walidacja na koniec
...
def main():
    ...
    parser.add_argument("--path_1h", required=True, help="Ścieżka do pliku z danymi 1H")
    parser.add_argument("--path_13c", required=True, help="Ścieżka do pliku z danymi 13C")
    parser.add_argument("--experiment_name", default="TransformerNMR_Hybrid")
    ...
    # Tworzymy katalog wyników, logger
    filename_1h = os.path.basename(args.path_1h)
    prefix = filename_1h.split("_")[0]
    res_dir = prefix + "-results-transformer-hybrid"
    os.makedirs(res_dir, exist_ok=True)

    log_file = os.path.join(res_dir, "script_transformer_hybrid.log")
    logging.basicConfig(
        level=logging.INFO,
    ...
    logger.info("Start skryptu (Transformer 1D, hybrid). Wyniki: %s", res_dir)

    try:
        # Wczytujemy dane
        X_nmr, y = load_nmr_data(args.path_1h, args.path_13c)
        X_nmr_t = torch.from_numpy(X_nmr).float()  # (N,1,2,200)
        ...
        logger.info("Start study.optimize (n_trials=%d)", args.n_trials)
        study.optimize(lambda tr: objective(tr, dataset, device), n_trials=args.n_trials)

        best_params = study.best_params
        best_value = study.best_value
        logger.info("Optuna zakończona. best_params=%s, best_value=%.4f", best_params, best_value)

        with mlflow.start_run(run_name="TransformerHybrid_10CV") as run:
            ...
            csv_optuna = os.path.join(res_dir, "optuna_trials_transformer_hybrid.csv")
            df_trials.to_csv(csv_optuna, index=False)
            mlflow.log_artifact(csv_optuna)

            # Wykres RMSE vs Trial ID
            plot_path = os.path.join(res_dir, "optuna_trials_rmse_transformer_hybrid.png")
            fig = plt.figure()
            plt.plot(df_trials["number"], df_trials["value"], marker="o", linestyle="-")
            plt.xlabel("Trial ID")
            plt.ylabel("RMSE (3CV)")
            plt.title("Optuna: RMSE vs Trial ID (Transformer Hybrid)")
            plt.grid(True)
            ...
            json_path = os.path.join(res_dir, "param_importances_transformer_hybrid.json")
            ...
            fig_imp_path = os.path.join(res_dir, "param_importances_transformer_hybrid.png")
            fig_real.savefig(fig_imp_path, dpi=300, bbox_inches="tight")
            mlflow.log_artifact(fig_imp_path)
            plt.close(fig_real)

            # 10CV z najlepszymi parametrami
            def best_model_func():
                ...
                model = TransformerRegressorHyb(trial_stub)
            ...
            logger.info("Start 10CV with best_params=%s", best_params)
            ...
            logger.info("10CV: RMSE=%.4f±%.4f, MAE=%.4f±%.4f",
            ...
            # Zapis metrics
            metrics_path = os.path.join(res_dir, "metrics_transformer_hybrid.csv")
            ...
            # hyperparams
            hyperparams_path = os.path.join(res_dir, "hyperparams_transformer_hybrid.csv")
            ...
            mlflow.pytorch.log_model(final_model, artifact_path="model_transformer_hybrid")
            mlflow.end_run()

        logger.info("Skrypt (Transformer 1D, hybrid) zakończony sukcesem.")

    except Exception as e:
        logger.exception("Błąd w trakcie działania skryptu (Transformer 1D hybrid).")
        sys.exit(1)
