...
# =================================================================================
# =================================================================================
# Model MLP Dual-Stream
# =================================================================================
class MLPDual(nn.Module):
    """
    Deep MLP with two parallel streams (¹H and ¹³C) merged into a shared embedding.
    Input: (B, 2, 200)
    Output: (B,)
    """
    def __init__(self, trial):
        ...
        def build_stream(prefix: str):
            ...
            for i in range(num_layers):
                out_dim = trial.suggest_int(f"{prefix}_hidden_dim_l{i}", 16, 1024, log=True)
                layers.extend([nn.Linear(in_dim, out_dim), act, nn.Dropout(p=dropout)])
                in_dim = out_dim
            return nn.Sequential(*layers), in_dim

        self.stream_h, h_out_dim = build_stream("h")
        self.stream_c, c_out_dim = build_stream("c")

        embed_dim = trial.suggest_int("embed_dim", 16, 512, log=True)
        self.fc_h_embed = nn.Linear(h_out_dim, embed_dim)
        self.fc_c_embed = nn.Linear(c_out_dim, embed_dim)

        final_dim = trial.suggest_int("final_hidden_dim", 16, 512, log=True)
        final_dropout = trial.suggest_float("final_dropout", 0.0, 0.6, step=0.1)
