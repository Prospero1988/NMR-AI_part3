...
Wykorzystuje Optuna do optymalizacji hiperparametrów, MLflow do logowania, oraz Early Stopping
zarówno w trakcie optymalizacji (pruning Optuny), jak i w 10CV.

Przykładowe użycie:
python train_hybrid_model.py \
    --path_1h /ścieżka/do/danych_1H.csv \
    --path_13c /ścieżka/do/danych_13C.csv \
    --path_fp /ścieżka/do/fingerprintów.csv \
    --experiment_name "Moje_eksperymenty" \
    --n_trials 20

Zakładamy, że:
- W pliku ¹H i ¹³C:
  * Pierwsza kolumna: MOLECULE_NAME (string)
  * Druga kolumna: etykieta (float, zakres -6..+6)
  * Kolejne 200 kolumn: cechy widma (liczby całkowite, głównie 0)
- W pliku fingerprintów:
  * Pierwsza kolumna: MOLECULE_NAME (string)
  * Druga kolumna: etykieta (float, ta sama co w widmach)
  * Kolejne 2048 kolumn: fingerprint ECFP4 (0/1)

Skrypt łączy dane po nazwie cząsteczki, a następnie tworzy hybrydową architekturę:
- CNN 2D (2 kanały: ¹H i ¹³C, z wymiarami (2, 1, 200)) do analizy widm
- MLP do analizy fingerprintów
- Łączenie wyjść obu modułów i przejście przez końcowe warstwy (regresja)

Optuna optymalizuje liczbę warstw, liczbę filtrów, dropout, lr, batch_size, itp.
Następnie najlepszy zestaw hiperparametrów jest używany w pełnej 10-krotnej walidacji krzyżowej.
Logujemy:
- Hiperparametry
- Metryki: RMSE, MAE, R^2, Pearson
- Wykres real vs. predicted (zebrany z całego 10CV)
- Model w formacie MLflow PyTorch

Uwaga: Ten kod może wymagać dłuższego czasu trenowania w zależności od rozmiarów danych.    
"""
...
class CNNModule(nn.Module):
    ...
    def __init__(self, trial):
        ...
        for i in range(num_conv_layers):
            ...
            conv_layers.append(nn.Conv2d(in_channels, out_channels, kernel_size=(1, kernel_size),
        ...
        # Wyjściowy wymiar po konwolucjach: (batch, out_channels, 1, szerokość)
        # Będziemy stosować global average pooling po szerokości, a potem fc
    ...
    def forward(self, x):
        # x: (batch_size, 2, 1, 200)
        x = self.conv(x)                # (B, out_channels, 1, W')
        x = torch.mean(x, dim=3)        # global average pooling po szerokości -> (B, out_channels, 1)
...
class MLPModule(nn.Module):
    ...
    def __init__(self, trial):
        ...
        dropout_mlp = trial.suggest_float("mlp_dropout", 0.0, 0.5, step=0.1)
...
class HybridModel(nn.Module):
    """
    Łączy CNNModule (NMR) i MLPModule (FP) i przechodzi przez finalne warstwy do regresji.
    """
...
def cross_validate(model_func, dataset, device, batch_size=64, n_folds=10, epochs=20):
    """
    :param model_func: funkcja (bez argumentów) tworząca (model, optimizer)
    :param dataset: torch Dataset z całościowymi danymi
    :param device: cpu/cuda
    :param batch_size: batch size
    :param n_folds: liczba foldów
    :param epochs: maksymalna liczba epok na fold
    :return: dict z metrykami i wektorami y_true_all, y_pred_all
    """
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(indices)):
        ...
        # Po zakończeniu pętli (wcześniej lub po epochs) liczymy finalne predykcje
        # (Można jeszcze raz wywołać `validate`, bo powyżej mamy już y_true, y_pred)
...
# =================================================================================
# FUNKCJA DO OPTUNA (z pruning i early stopping)
# =================================================================================
def objective(trial, dataset, device):
    """
    Optuna: używamy jednego folda (80/20) do szybkiej optymalizacji (pruning).
    Dodatkowo pokazane jest klasyczne early stopping w obrębie tego jednego splitu.
    """
    batch_size = trial.suggest_int("batch_size", 16, 256, log=True)
    max_epochs = 50  # maks. liczba epok; często w praktyce i tak przerwiemy wcześniej
    loss_fn = nn.MSELoss()

    # Losowo wydzielamy 80% train, 20% val
    indices = np.arange(len(dataset))
    ...
    for epoch in range(max_epochs):
        ...
        # Zgłaszamy metrykę do Optuny (pruning)
        trial.report(val_rmse, step=epoch)

        # Sprawdzamy, czy trial powinien być przerwany (pruned)
        if trial.should_prune():
            raise optuna.TrialPruned()

        # Klasyczne early stopping
        ...
        if patience_counter >= max_patience:
            # Kończymy trening w tym trialu, bo nie ma poprawy
            break
...
def main():
    ...
    parser.add_argument("--experiment_name", default="HybridNMR_FP_Experiment", help="Nazwa eksperymentu w MLflow")
    parser.add_argument("--n_trials", default=20, type=int, help="Liczba prób Optuna")
    parser.add_argument("--epochs_10cv", default=20, type=int, help="Maks. liczba epok do 10CV (z early stopping)")
    ...
    # Inicjujemy experiment w MLflow
    mlflow.set_experiment(args.experiment_name)

    # Optuna - optymalizacja hiperparametrów (pruning + early stopping)
    ...
    print("Najlepsze parametry z Optuna:")
    for k, v in best_params.items():
        print(f"  {k}: {v}")
    print(f"RMSE w walidacji (pojedynczy split 80/20): {best_value:.4f}")
    ...
    with mlflow.start_run(run_name="HybridCNN_MLP_Optuna_10CV"):
        ...
        # Tworzymy funkcję, która "zamraża" najlepsze parametry z Optuny i zwraca (model, optimizer)
        def best_model_func():
        ...
        # Kilka epok (tu np. tyle, co w 10CV), z early stopping w skali całego zbioru
        ...
        for epoch in range(args.epochs_10cv):
            ...
            # Pomiar RMSE na całym zbiorze (lub wydzielonym mini walidacyjnym, jeśli wolisz)
