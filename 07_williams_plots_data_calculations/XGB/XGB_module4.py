...
def process_file(csv_file, input_directory):
    ...
    try:
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            original_df = pd.read_csv(os.path.join(input_directory, csv_file))
            molecule_names = original_df.iloc[:, 0]  # Assuming first column is MOLECULE_NAME
            ...
            williams_csv = f"{os.path.splitext(csv_file)[0]}_williams_data.csv"
            williams_df.to_csv(williams_csv, index=False)
            mlflow.log_artifact(williams_csv)
