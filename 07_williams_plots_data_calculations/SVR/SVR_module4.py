...
def process_file(csv_file, input_directory):
    try:
        ...
        with mlflow.start_run(run_name=f"Training_{csv_file}_{int(time.time())}"):
            ...
            input_data_with_names = pd.read_csv(input_path)

            williams_df = pd.DataFrame({
                'MOLECULE_NAME': input_data_with_names.iloc[:, 0],
                'y_actual': y,
                'y_pred': y_pred_final
            })

            williams_file_name = f"{os.path.splitext(csv_file)[0]}_williams_data.csv"
            williams_df.to_csv(williams_file_name, index=False)
            mlflow.log_artifact(williams_file_name)
