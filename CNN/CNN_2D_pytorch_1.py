...
import tags_config_pytorch_CNN_2D
...
def load_data(csv1_path, csv2_path, target_column_name='LABEL'):
    try:
        ...
        assert np.array_equal(y1, y2), "Etykiety w obu plikach muszą być takie same!"
        ...
        X = np.stack((X1, X2), axis=1)  # Shape: (num_samples, 2, num_features)

        # Add an extra dimension to represent 'width'
        X = X[:, :, :, np.newaxis]  # Shape: (num_samples, 2, num_features, 1)
...
class Net(nn.Module):
    def __init__(self, trial):
        ...
        conv_layers = []
        in_channels = 2  # Since we have two channels (1H and 13C)
...
def objective(trial, csv1_path, csv2_path):
    try:
        # Load and process data
        X_train_full, y_train_full = load_data(csv1_path, csv2_path, target_column_name='LABEL')
...
def train_final_model(csv1_path, csv2_path, trial, csv_name):
    try:
        # Load data
        X_full, y_full = load_data(csv1_path, csv2_path, target_column_name='LABEL')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train 2D CNN models on two CSV files.')
    ...
    csv_name = f"{csv1_name}_{csv2_name}"

    print(f"\nProcessing files: {csv1_name} and {csv2_name}")

    try:
        # MLflow run for hyperparameter optimization
        with mlflow.start_run(
            run_name=f"Optimization_{csv_name}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}",
            tags=tags_config_pytorch_CNN_2D.mlflow_tags1
        ) as optuna_run:
            # Objective function wrapper
            def objective_wrapper(trial):
                return objective(trial, csv1_path, csv2_path)
        ...
        with mlflow.start_run(
            ...
            tags=tags_config_pytorch_CNN_2D.mlflow_tags2
        ) as training_run:
            # Train final model
            train_final_model(csv1_path, csv2_path, trial, csv_name)
