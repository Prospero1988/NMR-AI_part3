    ...
    try:
        ...
        with mlflow.start_run(run_name=f"Run_{csv_file}"):
            ...
            with mlflow.start_run(run_name=f"Final_Model_{csv_file}", nested=True):
                ...
                # Definiowanie schematu danych wejściowych
                input_schema = Schema([ColSpec("float", f"feature_{i}") for i in range(X.shape[1])])
                ...
                # Logowanie modelu w MLflow z ręcznie zdefiniowaną sygnaturą
                mlflow.keras.log_model(
                    model=model,
                    artifact_path="model",
                    signature=signature,
                    input_example=input_schema
                )
